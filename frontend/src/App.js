      ...
      useEffect(() => {
        const fetchData = async () => {
          const result = await axios(
            'http://127.0.0.1:5000/applicant_data',
          );
          for (let i = 0; i < result.data.length; i++) {
            if (!result.data[i].approval_status) {
                result.data[i]['approval_status'] = 0;
            }
        }
        ...
        <Router>
            <div >
                ...
                <Routes>
                    ...
                    <Route path = "/screening" element = { < Review  reviewList={realData} completeList={complete} data={[]} settings = { settings }/>} / >
